    ...
    public void run() {
        ...
        if (Main.setrandom == 0) {
            if (Other.config.getBoolean("BlackBox")) {
                ...
                random = randomnumber.nextInt(99)+1;
            }
        }
        else {
        ...
        for (String name : Main.lottery.keySet()) {
            ...
            switch (Main.lottery.get(play.getName())) {
                ...
                case 5: {
                    for (String player : Main.number.keySet()) {
                        int numberss = Main.number.get(player);
                        if (numberss < 10) {
        ...
        for (Player player : Bukkit.getServer().getOnlinePlayers()) {
            ...
            if (way == 0 && Main.data.containsKey(player.getName())) {
                ...
                for (String item2 : new ArrayList<String>(itemlist)) {
                    ...
                    int err = (allitemamount - Main.itemsizes.get(player.getName()) * 64) / multiple;
                    if (err > 0) {
