package pers.tany.superlottery.events;
...
import pers.tany.superlottery.conversation.*;
import pers.tany.superlottery.gui.*;
        ...
        if (a.getClickedInventory().getType() != InventoryType.PLAYER && p.getOpenInventory().getTitle().startsWith("§e下§a注§c界§b面") && (a.getSlot() <= 8 || a.getSlot() >= 36 || a.getSlot() == 9 || a.getSlot() == 18 || a.getSlot() == 27 || a.getSlot() == 17 || a.getSlot() == 26 || a.getSlot() == 35)) {
            ...
            if (a.getSlot() == 53) {
                ...
                while (grids <= 6) {
                    ...
                    if (checks != null) {
                        checksitem = CommonlyWay.GetItemData(checks);
                    }
                    if (checkss != null) {
                        checkssitem = CommonlyWay.GetItemData(checkss);
                    }
                    if (checksss != null) {
                        checksssitem = CommonlyWay.GetItemData(checksss);
                    }
                    if (checks != null) {
                        int number = 0;
                ...
                for (int from = 10; from <= 34; ++from) {
                    if (from != 17 && from != 18 && from != 26 && from != 27) {
                        ...
                        if (check != null) {
                            String itemID = CommonlyWay.GetItemData(check);
